...
from utils.tensor_utils import split_per_class, one_hot2class, resize_, one_hot
...
def ensure_onehot(vol):
    ...
    new_preds = torch.hstack([
        ...
        torch.zeros(len(zero_preds), n_nonbackground)
        ])
    vol[zero_preds[:, 0], 
            :, 
            zero_preds[:, 1], 
...
def load_data(args, debug=True): 
    ...
    gt_vols = [] if args.no_gt \
        else [(torch.from_numpy(np.asarray(x.dataobj))).type(torch.int64) for x in gt_nibs]

...
from nnunetv2.evaluation.metrics import ConfusionMatrix, ALL_METRICS
...
def evaluate_mixed_folder_structure(folder_with_gts: str, folder_with_predictions: str, labels: tuple, **metric_kwargs):
    """
    Writes a summary.json to folder_with_predictions. Handles ground truth files named 'GT.nii.gz' in subdirectories
    and predicted files in a flat directory.
    
    :param folder_with_gts: folder where the ground truth segmentations are saved (in subdirectories). Only 'GT.nii.gz' files are used.
    :param folder_with_predictions: folder where the predicted segmentations are saved (flat directory). Must be nifti files.
    :param labels: tuple of int with the labels in the dataset.
    :return:
    """
    def find_gt_files(folder):
        """
        Recursively find all 'GT.nii.gz' files in the ground truth folder and map them to their patient ID.
        """
        gt_files = {}
        for root, _, files in os.walk(folder):
            for file in files:
                if file == "GT.nii.gz":
                    patient_id = os.path.basename(root)  # Assume patient ID is the folder name
                    gt_files[patient_id] = os.path.join(root, file)
        return gt_files

    def find_pred_files(folder):
        """
        Find all predicted NIfTI files in the flat prediction folder.
        """
        pred_files = {}
        for file in os.listdir(folder):
            if file.endswith(".nii.gz"):
                patient_id = file.split(".nii.gz")[0].replace("patient_", "").strip()
                pred_files[patient_id] = os.path.join(folder, file)
        return pred_files

    # Find ground truth files (only 'GT.nii.gz' in subdirectories)
    files_gt = find_gt_files(folder_with_gts)
    # Find prediction files (flat directory)
    files_pred = find_pred_files(folder_with_predictions)

    # pprint.pprint(files_gt)
    # pprint.pprint(files_pred)

    # Ensure that all files in ground truth have a corresponding prediction file
    assert all([i in files_pred for i in files_gt]), "Files missing in folder_with_predictions"
    assert all([i in files_gt for i in files_pred]), "Files missing in folder_with_gts"

    # Create pairs of predicted and ground truth files for evaluation
    test_ref_pairs = [(files_pred[i], files_gt[i]) for i in files_gt]

    # Call your metric aggregation function (e.g., aggregate_scores)
    res = aggregate_scores(test_ref_pairs, json_output_file=os.path.join(folder_with_predictions, "summary_advanced.json"),
                           num_threads=8, labels=labels, **metric_kwargs)
    return res
...
def nnunet_evaluate_folder():
    ...
    return evaluate_mixed_folder_structure(args.ref, args.pred, args.l)


if __name__ == '__main__':
    RESULTS_FOLDER = '/home/scur2518/nnUNet_results/Dataset201_SegTHOR/'
    FOLD = 'fold_0/validation'
    preds = [
            #  join(RESULTS_FOLDER, 'nnUNetTrainerWandB_diceFocal_NoAug__nnUNetPlans__2d', FOLD),
            #  join(RESULTS_FOLDER, 'nnUNetTrainerWandB_diceFocal__nnUNetPlans__2d', FOLD),
             join(RESULTS_FOLDER, 'nnUNetTrainerWandB_diceFocal__nnUNetPlans__3d_fullres', FOLD),
             join(RESULTS_FOLDER, 'nnUNetTrainerWandB_diceFocal_NoAug__nnUNetPlans__3d_fullres', FOLD)]
    
    for prediction_path in preds:
        evaluate_mixed_folder_structure(
            '/home/scur2518/ai4mi_project/data/segthor_train/val',
            prediction_path,
            (1, 2, 3, 4), advanced=True
        )
        print('Finished evaluation for', prediction_path)